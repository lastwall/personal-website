...
const Blog: Page<BlogProps> = ({ posts }) => {
  ...
  return (
    <Section header="Blog">
      ...
      This is the blog page where I write programming, experiences and articles that I need to share. {' '}
      I wrote <b>{posts.length}</b> articles in total. and I'll share more soon ;)
      </p>
      ...
      <div>
        {!searchValue && (
          <Section header="Recent" id="recent-posts" className={null}>
            {recentPosts.map((post, i) => (
              <BlogPostCard post={post} key={i.toString()} />
            ))}
          </Section>
        )}
        <Section header="All" id="all-posts">
          {filteredPosts.length >= 1 ? (
            filteredPosts.map((post, i) => <BlogPostCard post={post} key={i.toString()} />)
          ) : searchValue ? (
            <h3 className="text-primary text-2xl">Nothing Found :{'('}</h3>
          ) : (
...
Blog.layoutProps = (ctx) => ({
  title: 'Blog',
  description: `This is the blog page where I write programming, experiences and articles that I need to share. I wrote ${ctx.posts.length} articles in total`,
});
